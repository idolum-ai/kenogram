...
an observed 68 GiB expanded footprint. SSH, Engram, and OpenClaw do not yet have
a reproducible `vfs` peak, so their `vfs` lanes fail closed instead of inventing a
...
| Command | Evidence |
|---|---|
| `make e2e-ssh` | Real SSH client, key rejection, no host listener, declared-name restriction, replacement, restart, and destruction |
